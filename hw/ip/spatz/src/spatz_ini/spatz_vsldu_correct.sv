...
`include "common_cells/registers.svh"
  ...
  logic [1:0] sparse_unit_effective_element_num = 2'd2;
  logic [2:0] sparse_unit_total_element_num     = 3'd4;
  ...
  always_comb begin: gather_output_comb
    ...
    if (spatz_req_valid && is_gather) begin
      if (vrf_rvalid_i[0] && vrf_rvalid_i[1]) begin
        ...
        gather_valid_d = sparse_out_full || gather_finished_d;
      end else if (half_gathered) begin
  ...
  logic sparse_idx_counter_en;
  logic sparse_idx_last;
  logic [3:0] sparse_idx_counter_d;
  logic [3:0] sparse_idx_counter_q;
  `FF(sparse_idx_counter_q, sparse_idx_counter_d, '0)

  logic [1:0] idx_bit_width      = 2'd2; 
  // idx_bit_width       : The number of bits of one index
  ...
  always_comb begin: vsldu_vreg_counter_proc
    ...
    if (vreg_counter_en) begin
      if (is_gather ? gather_finished_d : vreg_operation_last)
        // Reset the counter
        vreg_counter_d = '0;
      else 
    ...
    // Did we finish?
    vreg_operations_finished = (is_gather ? gather_finished_d : vreg_operation_last) && vreg_counter_en;

  end: vsldu_vreg_counter_proc
  
  always_comb begin: vsldu_sparse_counter_proc

    sparse_idx_counter_d      = sparse_idx_counter_q;
    sparse_idx_counter_en     =    (spatz_req.use_vs2 && vrf_re_o[0] && vrf_rvalid_i[0])
                                && (spatz_req.use_vs1 && vrf_re_o[1] && vrf_rvalid_i[1])
                                && (spatz_req.use_vd && (~gather_valid_d || (vrf_req_valid_d && vrf_req_ready_d)) || !spatz_req.use_vd);
    sparse_idx_last           = spatz_req_valid
                                && ((sparse_idx_counter_q+2) * idx_bit_width >= VRFWordWidth); 
    half_gathered             = sparse_idx_counter_d[1];
    sparse_out_full           = sparse_idx_counter_en && (!((sparse_idx_counter_q+2) % 4));
    gather_finished_d         = spatz_req_valid && is_gather && vreg_operation_last;
    
    if (sparse_idx_counter_en) begin
      if (sparse_idx_last || gather_finished_d) begin
        sparse_idx_counter_d = '0;
      end else begin
        sparse_idx_counter_d = sparse_idx_counter_q + 2'd2;
      end      
    end

  end: vsldu_sparse_counter_proc
    ...
    if (!is_gather) begin
      ...
      if (!is_vl_zero) begin
        ...
        if ((sparse_unit_effective_element_num == 1) && (sparse_unit_total_element_num == 2)) begin
          unique case (vrf_rdata_i[1][sparse_idx_counter_q * 2 +: 2]) 
            2'd0: data_low[63:0]  = vrf_rdata_i[0][63:0];
            2'd1: data_low[63:0]  = vrf_rdata_i[0][127:64];
            default:;
          endcase
          unique case (vrf_rdata_i[1][(sparse_idx_counter_q+1) * 2 +: 2]) 
        ...
        unique case (sparse_idx_counter_q % 4)
          4'd0 : gather_partial_data[127:0]   = {data_high[63:0], data_low[63:0]};
          4'd2 : gather_partial_data[255:128] = {data_high[63:0], data_low[63:0]};
        endcase
