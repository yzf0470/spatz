...
def emit_sparsemv_layer(name="sparsemv", **kwargs):
    ...
    if dtype != "char":
        ...
        layer_str += (
            f'static {"short"} {name}_packed_idx_A_dram [{m} * {n//16}] __attribute__((section(".data"))) = '
            + array_to_cstr(packed_idx_mat_A)
            + ";\n\n\n"
        )
        layer_str += (
            f'static {"short"} {name}_packed_idx_B_dram [{n//16}] __attribute__((section(".data"))) = '
...
def main():
    ...
    vec_B, idx_vec_B, packed_idx_vec_B         = mat_A[0:param["N"]], idx_mat_A[0:(param["N"])//2], packed_idx_mat_A[0:(param["N"])//16]
